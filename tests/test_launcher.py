...
from goodoldgalaxy import launcher
from goodoldgalaxy.game import Game


class Test(TestCase):
    def test1_determine_launcher_type(self):
        files = ['thumbnail.jpg', 'docs', 'support', 'game', 'start.sh', 'goodoldgalaxy-dlc.json', 'gameinfo']
        exp = "start_script"
    ...
    def test3_determine_launcher_type(self):
        files = ['thumbnail.jpg', 'docs', 'support', 'unins000.exe', 'goodoldgalaxy-dlc.json', 'gameinfo']
        exp = "windows"
        obs = launcher.determine_launcher_type(files)
        self.assertEqual(exp, obs)
    ...
    def test4_determine_launcher_type(self, mock_shutil_which):
        mock_shutil_which.return_value = True
        files = ['thumbnail.jpg', 'docs', 'support', 'dosbox', 'goodoldgalaxy-dlc.json', 'gameinfo']
        exp = "dosbox"
        obs = launcher.determine_launcher_type(files)
        self.assertEqual(exp, obs)

    def test5_determine_launcher_type(self):
        files = ['thumbnail.jpg', 'docs', 'support', 'game', 'goodoldgalaxy-dlc.json', 'gameinfo']
        exp = "final_resort"
        obs = launcher.determine_launcher_type(files)
        self.assertEqual(exp, obs)
    ...
    def test1_get_windows_exe_cmd(self, mock_glob):
        ...
        files = ['thumbnail.jpg', 'docs', 'support', 'game', 'goodoldgalaxy-dlc.json', 'start.exe', 'unins000.exe']
    ...
    @mock.patch('goodoldgalaxy.launcher.get_execute_command')
    ...
    @mock.patch('goodoldgalaxy.launcher.check_if_game_start_process_spawned_final_process')

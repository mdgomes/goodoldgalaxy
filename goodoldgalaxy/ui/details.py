...
class Details(Gtk.Viewport):
    ...
    def set_game(self, game: Game = None):
        ...
        if game is None or self.game == game:
            return
        # unregister state listeners from previous shown game
        if self.game is not None:
            # unregister listener for old game
            self.game.unregister_state_listener(self.__game_state_listener)
            for dlc in self.game.dlcs:
                dlc.unregister_state_listener(self.__dlc_state_listener)
        ...
        if "description" in self.response:
            ...
            try:
                desc = self.__cleanup_html(self.response["description"]["full"])
        ...
        if self.downloadstree is not None and self.downloadstree.get_model() is not None:
            # clear the store
            self.downloadstree.get_model().get_model().clear()
            # remove columns
            while len(self.downloadstree.get_columns()) > 0:
                self.downloadstree.remove_column(self.downloadstree.get_columns()[0])
        if self.goodiestree is not None and self.goodiestree.get_model() is not None:
            # clear the store
            self.goodiestree.get_model().clear()
            # remove columns
            while len(self.goodiestree.get_columns()) > 0:
                self.goodiestree.remove_column(self.goodiestree.get_columns()[0])
        # downloads
        didx=0
        gidx=0
        #GdkPixbuf.Pixbuf
        downloads_store = Gtk.ListStore(str,str,str,str,str,str,str,str,int,str,str)
        ...
        for dlc in self.game.dlcs:
            dlc.register_state_listener(self.__dlc_state_listener)
    ...
    def __game_state_listener(self, game:Game, state):
        ...
        if state == game.state.DOWNLOADABLE:
            # this progress function is only used for the main game
            for row in self.downloadstree.get_model():
                if row[3] != _("Installer"):
                    continue
                row[6] = "N/A"
            for dlc in game.dlcs:
                self.__dlc_state_listener(dlc, state)
    
    def __dlc_state_listener(self, dlc:Game, state):
        # stuff to change
        # Update DLC Installer row
        for row in self.downloadstree.get_model():
            if row[3] != "DLC "+_("Installer"):
                continue
            if state == dlc.state.DOWNLOADABLE:
                row[6] = "N/A"
            elif state == dlc.state.INSTALLED:
                row[6] = _("Install")
    
    def load_achievements(self):
        # hide tab
        self.notebook.get_nth_page(2).hide()
        # clear existing achievements if existing
        while len(self.achievements_box.get_children()) > 0:
            child = self.achievements_box.get_children()[0]
            self.achievements_box.remove(child)
            # destroy child which is an image
            child.destroy()
    ...
    def __download_file(self,widget = None,link:str = None,fname:str = None):
        download_info = None;
        if link == None:
            # assume link is for game
            download_info = self.api.get_download_info(self.game)
            for file_info in download_info['files']:
                link = self.api.get_real_download_link(file_info["downlink"])
                break
        ...
        file_name = self.game.get_stripped_name() if fname is None else fname
        ...
        if response == Gtk.ResponseType.ACCEPT:
            if download_info is None:    
                ...
                downloads = []
                down_count = len(download_info["files"])
                down_idx = 0
                for file_info in download_info['files']:
                    download = Download(
                        url=self.api.get_real_download_link(file_info["downlink"]),
                        title=download_info["name"]+" ("+download_info["id"]+"_"+file_info["id"]+")",
                        save_location=dialog.get_filename()+"_"+file_info["id"],
                        associated_object=self.game,
                        out_of_amount=down_count,
                        number=down_idx
                    )
    ...
    def __append_to_list_store_for_download(self,store,item,download_type,is_goodie=False,state: str="", product_id: int = None, file_size: int = None):
        ...
        bonus_count = None
        if is_goodie:
            bonus_type = BONUS_TYPES[item["type"].strip().lower()]
        ...
        if "files" in item:
            ...
            for file in files:
                ...
                if is_goodie:
                    ...
                    os_label = self.__create_os_label(download_os)
                    store.append([download_name,os_label,download_language,download_type,file_size,download_version,state,file["downlink"],product_id,"{}".format(file["size"]),item["os"]])
        return links
    
    def __create_os_label(self, os: str) -> str:
